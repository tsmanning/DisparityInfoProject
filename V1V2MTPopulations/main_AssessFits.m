...
V1 = load([analyDir,'resultsV1_final.mat']);
V2 = load([analyDir,'resultsV2_final.mat']);
MT = load([analyDir,'resultsMT_final.mat']);
...
%% Alternatively, chain all the subsampling directions together
if doAllSubsampling
    ...
    [r2Inds,eccInds] = subsampleCells(V1,V2,MT);

    V1.x_pos   = V1.x_pos(eccInds.V1);
    V1.y_pos   = V1.y_pos(eccInds.V1);
    V1.FI      = V1.FI(eccInds.V1,:);
    V1.allresp = V1.allresp(eccInds.V1,:);
    V1.P       = V1.P(eccInds.V1,:);
    V1.maxsps  = V1.maxsps(eccInds.V1);
    V1.pref_disp = V1.pref_disp(eccInds.V1);

    V2.x_pos   = V2.x_pos(eccInds.V2);
    V2.y_pos   = V2.y_pos(eccInds.V2);
    V2.FI      = V2.FI(eccInds.V2,:);
    V2.allresp = V2.allresp(eccInds.V2,:);
    V2.P       = V2.P(eccInds.V2,:);
    V2.maxsps  = V2.maxsps(eccInds.V2);
    V2.pref_disp = V2.pref_disp(eccInds.V2);

    MT.x_pos   = MT.x_pos(eccInds.MT);
    MT.y_pos   = MT.y_pos(eccInds.MT);
    MT.FI      = MT.FI(eccInds.MT,:);
    MT.allresp = MT.allresp(eccInds.MT,:);
    MT.P       = MT.P(eccInds.MT,:);
    MT.maxsps  = MT.maxsps(eccInds.MT);
...
for p = 1:6
    ...
    if p == 5
        distributionPlot([log(V1.P(:,p))],'xValues',3,'color',ColorIt('b'),'histOpt',2,...
            'divFactor',2,'showMM',6,'xNames',{'V1'},'xyOri','flipped');
        distributionPlot([log(V2.P(:,p))],'xValues',2,'color',ColorIt('g'),'histOpt',2,...
            'divFactor',2,'showMM',6,'xNames',{'V2'},'xyOri','flipped');
        distributionPlot([log(MT.P(:,p))],'xValues',1,'color',ColorIt('r'),'histOpt',2,...
            'divFactor',8,'showMM',6,'xNames',{'MT'},'xyOri','flipped');
...
f4a = figure; 
f4a.Position = [100 100 650 600];
hold on;

% % baseline sps
% subplot(1,3,1); hold on; title('Baseline spike rate (sps)');
% distributionPlot([V1.P(:,1)],'xValues',3,'color',ColorIt('b'),'histOpt',1,...
%     'showMM',6,'xNames',{'V1'},'xyOri','flipped');
% distributionPlot([V2.P(:,1)],'xValues',2,'color',ColorIt('g'),'histOpt',1,...
%     'showMM',6,'xNames',{'V2'},'xyOri','flipped');
% distributionPlot([MT.P(:,1)],'xValues',1,'color',ColorIt('r'),'histOpt',1,...
%     'showMM',6,'xNames',{'MT'},'xyOri','flipped');
...
% max sps
title('Maximum spike rate (sps)');
distributionPlot(V1.maxsps','xValues',3,'color',ColorIt('b'),'histOpt',1,...
    'showMM',6,'xNames',{'V1'},'xyOri','flipped');
distributionPlot(V2.maxsps','xValues',2,'color',ColorIt('g'),'histOpt',1,...
    'showMM',6,'xNames',{'V2'},'xyOri','flipped');
distributionPlot(MT.maxsps','xValues',1,'color',ColorIt('r'),'histOpt',1,...
    'showMM',6,'xNames',{'MT'},'xyOri','flipped');
...
f4b = figure; 
f4b.Position = [100 100 650 600];
hold on;

% preferred disparity
title('Preferred disparity (deg)');
distributionPlot(V1.pref_disp','xValues',3,'color',ColorIt('b'),'histOpt',1,...
    'showMM',6,'xNames',{'V1'},'xyOri','flipped');
distributionPlot(V2.pref_disp','xValues',2,'color',ColorIt('g'),'histOpt',1,...
    'showMM',6,'xNames',{'V2'},'xyOri','flipped');
distributionPlot(MT.pref_disp','xValues',1,'color',ColorIt('r'),'histOpt',1,...
    'showMM',6,'xNames',{'MT'},'xyOri','flipped');
xlim([-2 2])
axis square; box on;
set(gca,'ytick',[1 2 3],'YTickLabel',{'MT','V2','V1'},'fontsize',20);
...
saveas(f3,[topDir,'plots/AssessFits/Fit_Params.svg']);

% Nonparametric fit characteristics
saveas(f4a,[topDir,'plots/AssessFits/Fit_Characteristics_a.svg']);
saveas(f4b,[topDir,'plots/AssessFits/Fit_Characteristics_b.svg']);
